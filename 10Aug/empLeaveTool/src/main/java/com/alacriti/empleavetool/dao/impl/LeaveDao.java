...
public class LeaveDao extends BaseDao {
	...
	public ArrayList<LeaveVo> historyOfLeaves(LeaveVo leavevo) throws DAOException {
		Connection conn = null;
		String query = null;
		String emp_id;
		Date date_applied;
	...
	String leave_id;
		...
		try {
			...
			query = "Select emp_id,date_applied,leave_to_date,no_days,leave_reason,last_modify_time,leave_id,leave_tag from amans_empleaveportal_leave_tbl where emp_id=?";
			...
			while (rs1.next()) {
				...
				leavevolist.add(new LeaveVo(leave_to_date,date_applied,emp_id,no_days,leave_reason,leave_tag,leave_id,last_modify_time));
